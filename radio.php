...
function start_player($stationUrl, $db){
    ...
    stop_player($db);
    $stationUrl = urldecode($stationUrl);
    $command = "cvlc --intf http --http-port 9090 --http-password password --volume 90 $stationUrl";
    $command = escapeshellcmd($command);
...
function start_player_west_coast($stationUrl, $db, $secondsIntoFile){
    ...
    $command = "cvlc --intf http --http-port 9090 --http-password password --volume 90 --start-time $secondsIntoFile $stationUrl";
    $command = escapeshellcmd($command);
    exec($command . " > /dev/null &");
